...
class SqlModelDatabase(AbstractDatabase["SqlModelDatabase"]):
    ...
    class Operations(AbstractDatabase.Operations):
        ...
        def run_migrations(self) -> None:
            ...
            self._drop_column_if_exists("clip", "start_ms")
            self._drop_column_if_exists("clip", "end_ms")
            self._drop_column_if_exists("clip", "utterance_start")
            self._drop_column_if_exists("clip", "utterance_end")
        ...
        def _migrate_table(
            ...
            for column_name, column_type in new_columns:
                if column_name not in existing_columns:
                    with Session(self.database.engine) as session:
                        if "NOT NULL" in column_type and self._table_has_existing_rows(
                            session, table_name
                        ):
                            default_value = self._get_default_value(column_type)
                            alter_statement = f'ALTER TABLE "{table_name}" ADD COLUMN {column_name} {column_type} DEFAULT {default_value}'
                        else:
        ...
        def _table_has_existing_rows(self, session: Session, table_name: str) -> bool:
            result = session.execute(
                text(f'SELECT EXISTS (SELECT 1 FROM "{table_name}" LIMIT 1)')
            )
            return result.scalar()

        def _get_default_value(self, column_type: str) -> str:
            base_type = column_type.split()[0].upper()
            if "UUID" in base_type:
                return "gen_random_uuid()"
            elif base_type == "INTEGER":
                return "0"
            elif base_type in ["VARCHAR", "TEXT"]:
                return "''"
            elif base_type == "BOOLEAN":
                return "FALSE"
            elif base_type == "DOUBLE" or base_type == "DOUBLE PRECISION":
                return "0.0"
            else:
