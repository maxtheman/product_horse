...
# %% auto 0
__all__ = ['logger', 'STRINGS_TO_SANITIZE', 'ValidString', 'ValidPath', 'T', 'P', 'DBType', 'TModelOut', 'validate_file_path',
...
from sqlalchemy.exc import PendingRollbackError, DBAPIError, SQLAlchemyError
...
class DatabaseProtocol(Protocol):
    ...
    class Operations(ABC):
        def check_db_connection(self, session: Session) -> bool: ...
    @property
    ...
    def decorator(
        ...
        def wrapper(self: DBType, *args: P.args, **kwargs: P.kwargs) -> T:
            ...
            with self.get_session_for_employee() as session:
                ...
                try:
                    if not self.operations.check_db_connection(session):
...
class AbstractDatabase(ABC, DatabaseProtocol, Generic[DBType]):
    ...
    class Operations(DatabaseProtocol.Operations):
        @abstractmethod
        def check_db_connection(self, session: Session) -> bool:
...
class SqlModelDatabase(AbstractDatabase["SqlModelDatabase"]):
    ...
    def _save(
        ...
        if joinedload_keys:
            model_to_return = session.exec(
                select(output_model)
                .options(joinedload(*joinedload_keys))  # type: ignore
                .where(output_model.id == valid_model.id)  # type: ignore
            ).first()
        else:
            model_to_return = session.exec(
                select(output_model).where(output_model.id == valid_model.id)  # type: ignore
            ).first()
        if model_to_return is None:
            raise ValueError(
                f"{output_model.__name__} with id {valid_model.id} not found"  # type: ignore
            )
    ...
    def save_file_metadata(
        ...
        return self._save(
            ...
            metadata,
            FileMetadata,
            joinedload_keys=[FileMetadata.transcription], # type: ignore
        )
    ...
    def get_all_unique_transcriptions(
        ...
        if mode == "file_name":
            unique_metadata_query = session.exec(
                select(FileMetadata).distinct(col(FileMetadata.file_path))
            )
