...
# %% auto 0
__all__ = ['logger', 'save_mp4_animation_to_file', 'put_words_over_video_subset', 'write_video', 'write_video_with_audio',
           'render_clip', 'render_clips', 'create_video_from_clips', 'filter_clips_by_utterance_segments',
...
def write_video_with_audio(output_path: str, video_clip: VideoClip | VideoFileClip | CompositeVideoClip, buffer_size: int = 100):
    encoders = subprocess.check_output(['ffmpeg', '-encoders'], universal_newlines=True)
    
    #Check for preferred encoders
    if 'h264_nvenc' in encoders:
        encoder = 'h264_nvenc'
    elif 'h264_cuvid' in encoders:
        encoder = 'h264_cuvid'
    ...
    video_fps = video_clip.fps # type: ignore
    if video_fps is None:
        video_fps = 24
    audio_fps_default = 44100

    # Extract audio to a temporary file if it exists
    temp_audio_file = None
    if video_clip.audio is not None: # type: ignore
        temp_audio_file = tempfile.NamedTemporaryFile(suffix=".wav", delete=False)
        if not hasattr(video_clip, 'audio'):
            raise ValueError("Video clip has no audio")
        if not hasattr(video_clip.audio, 'fps'): # type: ignore
            video_clip.audio.write_audiofile(temp_audio_file.name, fps=audio_fps_default) # type: ignore
        else:
            video_clip.audio.write_audiofile(temp_audio_file.name, fps=video_clip.audio.fps) # type: ignore
    ...
    if encoder in ['h264_nvenc', 'h264_cuvid']:
    ...
    command.extend([
        ...
        '-s', f'{width}x{height}',
        '-pix_fmt', 'rgb24',
        '-r', str(video_fps), # type: ignore
        '-i', '-'  # Video input from pipe
    ])

    if temp_audio_file:
        command.extend(['-i', temp_audio_file.name])  # Audio input from file

    if encoder == 'h264_nvenc':
        command.extend([
            '-c:v', 'h264_nvenc',
            '-preset', 'p1',  # Fastest preset for NVENC
            '-b:v', '5M',  # Set bitrate to 5 Mbps (adjust as needed)
            '-maxrate', '10M',  # Maximum bitrate
            '-bufsize', '15M',  # VBV buffer size
            '-rc', 'vbr',  # Variable bitrate mode
            '-cq', '19',  # Constant quality factor (lower is better quality)
            '-pix_fmt', 'yuv420p',
            '-movflags', 'faststart'
        ])
    elif encoder == 'h264_cuvid':
        command.extend([
            '-vf', 'scale_npp=-1:720',  # Scale to 720p using NVIDIA Performance Primitives
    ...
    if process.stdin is None:
        raise ValueError("Failed to create stdin for FFmpeg")

    # Pre-allocate video buffer
    video_buffer = np.empty((buffer_size, height, width, 3), dtype=np.uint8)
    buffer_index = 0

    # Write video frames - 90% OF FUNCTION RUNTIME
    for frame in video_clip.iter_frames(fps=video_fps, dtype="uint8"):
        video_buffer[buffer_index] = frame
        buffer_index += 1

        if buffer_index == buffer_size:
            process.stdin.write(video_buffer.tobytes())
            buffer_index = 0

    # Write any remaining frames in the buffer
    if buffer_index > 0:
        process.stdin.write(video_buffer[:buffer_index].tobytes())
...
async def render_clips(
    ...
    for clip in clip_list:
        ...
        try:
            ...
            logger.error(f"Error rendering clip {clip.id}: {str(e)}")
            logger.error(f'filepath: {clip.file_path}')
            raise e
    return final_clips
...
def create_video_from_clips(
    ...
    try:
        with final_video as video_render:
            ...
            write_video_with_audio(temp_file_path, video_render)
...
async def create_video_from_utterances(
    ...
    with local_file_system.temporary_user_directory(user) as temp_directory:
        ...
        logger.debug(f"Clips rendered: {len(clips_to_render)}")
        with tempfile.NamedTemporaryFile(delete=False, suffix=".mp4") as temp_file:
            ...
            with local_file_system.file_stream(video_path, "rb") as source_stream:
                name = final_destination.split("/")[-1]
                try:
                    ...
                    logger.error(f"Error creating video at {final_destination}: {e}")
                    video.render_status = RenderStatus.failed
